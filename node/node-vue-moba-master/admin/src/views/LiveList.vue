      ...
      <el-table-column fixed="right" label="操作" width="180">
        <template slot-scope="scope">
          <el-button
            type="text"
            size="small"
            @click="$router.push(`/lives/edit/${scope.row._id}`)"
          >编辑</el-button>
...
export default {
  ...
  methods: {
    async fetch() {
      const res = await this.$http.get("rest/lives");
      this.items = res.data;
    },
    remove(row) {
      this.$confirm(`是否确定要删除 "${row.nickname}"`, "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      }).then(async () => {
        const res = await this.$http.delete(`rest/lives/${row._id}`);
        this.$message({
          type: "success",
          message: "删除成功!"
        });
